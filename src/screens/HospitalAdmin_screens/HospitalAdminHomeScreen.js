...
const HospitalAdminHomeScreen = ({ history }) => {
                  ...
                  <Card.Body>
                    ...
                    <> {show2 ? (<ListGroup className="list-group-flush">
                      ...
                      {Array.from({ length: bedInfo["results"]["NormalBed"].length }).map(
                        (_, i) => (
                          <>  {bedInfo["results"]["NormalBed"][`${i}`]["IsOccupied"] === true ? (<ListGroup.Item style={{ color: "red", textAlign: "center" }}>{bedInfo["results"]["NormalBed"][`${i}`]["BedID"]}    {bedInfo["results"]["NormalBed"][`${i}`]["IsOccupied"] === true ? ("Occupied") : ("Not Occupied")}</ListGroup.Item>) : (<ListGroup.Item style={{ color: "green", textAlign: "center" }}>{bedInfo["results"]["NormalBed"][`${i}`]["BedID"]} {bedInfo["results"]["NormalBed"][`${i}`]["IsOccupied"] === true ? ("Occupied") : ("Not Occupied")}</ListGroup.Item>)} </>
                        )
                      )}
